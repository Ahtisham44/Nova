...
A comprehensive, systematic design foundation and React 19 component library engineered for building high-trust financial products.
...
## Design Token Architecture (`src/tokens/`)

The token system is structured into three distinct tiers:
1. **Primitive (`primitives.ts`, `scale.ts`):** Raw design values (`--color-nova-600: #16A34A`, `--space-4: 16px`).
2. **Semantic (`colors.ts`):** Role-based aliases (`--color-primary: var(--color-nova-600)`, `--color-surface: var(--color-neutral-50)`).
3. **Component (`components.ts`):** Usage-specific tokens (`--button-radius: var(--radius-md)`).
